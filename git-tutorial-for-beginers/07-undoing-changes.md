...
Представим, что нам не нужно было делать этих изменений, а значит нужно вернуться к старой версии файла. Это можно сделать с помощью команды **`git checkout`**.
Обычно эта команда используется для работы с ветками, она требует git выполнить следующее: "Иди в репозиторий, возьми 'нечто' которое я тебе передал в параметрах, и сделай так, чтобы это 'нечто' в рабочей директории выглядело так же как 'нечто' в репозитории". Так что, если "нечто" это ветка, то git выполняет действия над веткой, а если файл – то над файлом.

Чтобы сказать git, что при выполнении команды **`git checkout`** мы хотим работать с файлом, нужно в данной команде поставить две черты, например: **`git checkout -- index.html`**. Это даст git подсказку, что мы хотим работать с файлом **index.html** в текущей ветке. Данной командой мы будто говорим git "Пойди в репозиторий, посмотри как там выглядит файл **index.html** и сделай так, чтобы файл **index.html** в текущей директории выглядел так же". Выполним эту команду и отменим изменения в указанном файле:
...
Для начала сделаем некоторые изменения в файле **resources.html** и добавим их в индекс. Статус git:

![](pics/07-04.png)

Чтобы отменить сделанные изменения которые уже находятся в индексе, но при этом, чтобы они остались в рабочей директории, для файла **resources.html** необходимо выполнить команду **`git reset HEAD resources.html`**. Эта команда говорит git, чтобы он посмотрел на что указывает **HEAD** (а **HEAD** указывает на последний коммит в текущей ветке), и привёл состояние файла **resources.html** в индексе текущей ветки к такому состоянию, которое соответствует состоянию файла **resources.html** на момент последнего коммита.

Выполнение данной процедуры может понадобиться если вы по ошибке добавили в индекс несвязанные между собой изменения файлов, которые хотели разбить на два пакета изменений, сделав два коммита.
...
###Отмена последнего коммита (amending commits)

Закрепим в индексе изменения из прошлого шага и добавим их в репозиторий. 
Закрепляем:
...
Теперь представим, что мы что-то не доделали, и необходимо добавить эти изменения в общий пакет последнего коммита. Сделаем изменения. Статус git покажет следующее:

![](pics/07-09.png)

Для того чтобы добавить эти изменения в последний коммит, необходимо сначала закрепить сделанные изменения в индексе:
...
А дальше выполнить команду **`git commit --amend -m "Rearrange the items to bring on a trip"`**. Это позволит добавить новые изменения к последнему коммиту:
...
Лог покажет, что у нас только один коммит с указанным сообщением, так что всё сработало как и задумывалось:
...
После изменения сообщения последнего коммита, git будет показывать следующий лог:

![](pics/07-15.png)

**Примечание!** При изменении сообщения последнего коммита меняется значение контрольной суммы, так как контрольная сумма зависит от сообщений коммитов, временных маркеров и прочего.
...
**Первый способ отменить последний коммит** это вручную сделать изменения отменяющие изменения в последнем коммите и сделать новый коммит.

###Отмена последнего коммита при помощи **`git checkout`**

**Второй способ отменить последний коммит** это отменить их при помощи git. Например, представим, что нам необходимо отменить коммит "Rearrange the items to bring on an outdoor trip", вернувшись к состоянию файла **resources.html** такому, которое было на момент когда последним коммитом был коммит "Removed contractions from contact page text" с контрольной суммой **4d55091...**. Для этого необходимо выполнить команду: **`git checkout 4d55091 -- resources.html`**

![](pics/07-16.png)

Как видим, статус git сообщает, что мы вернулись к состоянию, когда изменения файла **resources.html** находятся в индексе (но уже отсутствуют в репозитории в виде коммита):
...
Видно какие строки будут удалены, а какие добавлены – именно таким было первоначальное состояние файла, перед реорганизацией списка.

Теперь если сделать коммит, то мы вернём состояние файла **resources.html** к первоначальному. Хорошей практикой для сообщения коммита который устраняет другой коммит, указывать контрольную сумму устраняемого коммита.

Чтобы продолжить обучение, вернёмся к состоянию, которое было до начала отмены последнего коммита. Это позволит сделать выполнить те же действия третим способом.
...
###Отмена последнего коммита при помощи **`git revert`**

Чтобы выполнить то же действие, что и в прошлом уроке, - вернуться к старому состоянию файла **resource.html**, можно использовать команду **`git revert`**. Данная команда позволяет отменить указываемый коммит.

Выполняем команду **`git revert HASH`** (где **HASH** – контрольная сумма отменяемого коммита:

![](pics/07-23.png)

Важно понимать, что отменяются и все коммиты идущие за указанным коммитом.

Во время выполнения команды **`git revert HASH`** откроется редактор по умолчанию с требованием ввести сообщение для создаваемого отменяющего коммита (можно оставить сообщение созданное git, оно достаточно точно описывает проделываемое действие):
...
А лог git-а показывает новый коммит:

![](pics/07-26.png)

**Примечание!** Если делать возвращение (**revert**) к не последнему коммиту, а например, предпоследнему, то возникнет конфликт слияния. Чтобы пройти путь без конфликтов, можно возвращаться по одному коммиту назад, хотя при этом и создаются новые коммиты (возвращающие/отменающие).
...
###Использование **`git reset`** для отмены множества коммитов

Команда **`git reset`** перебрасывает указатель **HEAD** на требуемый коммит. Обычно git самостоятельно управляет указателем **HEAD**, это происходит когда мы делаем коммит, в это время git переставляет указатель **HEAD** на новый/последний коммит. А командой **`git reset`** мы говорим git, что сами хотим управлять указателем **HEAD** и поставить его на определённый коммит.

У данной команды есть три ключа, которые будут управлять состоянием рабочей директории и индекса: ключ **`--soft`**, ключ **`--mixed`** и ключ **`--hard`**.

- Команда **`git reset`** с ключом **`--soft`** передвигает указатель **HEAD**, при этом не изменяя индекс и рабочую директорию.

- Команда **`git reset`** с ключом **`--mixed`**, который является ключом по умолчанию, передвигает указатель **HEAD**, при этом изменяет индекс, чтобы он соответствовал репозиторию. Но не изменяет рабочую директорию.

- Команда **`git reset`** с ключом **`--hard`** передвигает указатель **HEAD**, при этом изменяет индекс и рабочую директорию так, чтобы они соответствовали репозиторию.
